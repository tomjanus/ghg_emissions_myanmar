...
def get_numeric_cols(data: pd.DataFrame) -> List[str]:
    """ """
    return list(data.select_dtypes(exclude=['object', 'bool', 'category']).columns)


def get_categorical_cols(data: pd.DataFrame) -> List[str]:
    """ """
    return list(data.select_dtypes(include=['object', 'bool', 'category']).columns)
...
# Custom transformers
class ProfileDropper(BaseEstimator, TransformerMixin):
    """Drop emission profiles from data"""
    
    def fit(self, X, y=None):
        return self
    
    def transform(self, X: pd.DataFrame):
        """Note: operate on the object itself, not on a copy"""
        X_new = X.drop(columns = [f'co2_profile_{ix}' for ix in range(0,10)], errors='ignore')
        X_new = X_new.drop(columns = [f'ch4_profile_{ix}' for ix in range(0,10)], errors='ignore')
        X_new = X_new.drop(columns = [f'n2o_profile_{ix}' for ix in range(0,10)], errors='ignore')
        X_new = X_new.drop(columns = [f'year_profile_{ix}' for ix in range(0,10)], errors='ignore')
        return X_new
    

class ReplaceTempProfileWithMean(BaseEstimator, TransformerMixin):
    """Replaces 12x1 temp vector with one mean temperature column"""
    
    def fit(self, X, y=None):
        return self
    
    def transform(self, X: pd.DataFrame):
        monthly_temp_cols = [f'monthly_temps_{ix}' for ix in range(0,12)]
        X_new = X.copy()
        X_new['ave_temp'] = X_new[monthly_temp_cols].mean(axis=1)
        X_new.drop(columns=monthly_temp_cols, inplace=True)
        return X_new


class ReduceResAreaFractions(BaseEstimator, TransformerMixin):
    """ """
    ...
    def transform(self, X: pd.DataFrame):
        """ """
        res_area_cols = [f'res_area_fractions_{ix}' for ix in range(0,27)]
        new_res_area_cols = [f'res_area_fractions_red_{ix}' for ix in range(0,9)]
        X_new = X.copy()
        for ix, new_col in enumerate(new_res_area_cols):
            old_ixs = [ix, ix + 9, ix + 18]
            X_new[new_col] = sum([X_new[res_area_cols[ix]] for ix in old_ixs])
        X_new.drop(columns=res_area_cols, inplace=True, errors='ignore')
        return X_new
...
class ColumnDropper(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame):
        X_new = X.drop(columns=self.columns, errors='ignore')
        return X_new
    
    
class SplitScenario(BaseEstimator, TransformerMixin):
    """ """
    def fit(self, X, y=None):
        return self
    ...
    def transform(self, X: pd.DataFrame):
        X_new = X.copy()
        X_new[['Soil', 'Landuse intensity', 'Treatment']] = \
            X_new['Scenario'].str.split('_', expand=True)
        return X_new

    
class StaticColRemover(BaseEstimator, TransformerMixin):
    """ """
    def fit(self, X, y=None):
        return self
    
    def transform(self, X: pd.DataFrame):
        numeric_data = X.select_dtypes(exclude=['object', 'category'])
        constant_columns = numeric_data.columns[numeric_data.std() == 0]
        X_new = X.copy()
        X_new = X.drop(columns=constant_columns)
        return X_new
...
class CategoricalColumnSelector(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        #print(get_categorical_cols(X))
        return self
     
    def transform(self, X):
        return X[get_categorical_cols(X)]
...
class NumericalColumnSelector(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        #print(get_numeric_cols(X))
        return self
     
    def transform(self, X):
        return X[get_numeric_cols(X)]
