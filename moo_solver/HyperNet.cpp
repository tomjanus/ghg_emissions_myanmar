...
HyperNet::HyperNet(const HyperNet &src): num_nodes(src.num_nodes), num_edges(src.num_edges), 
...
HyperNet &HyperNet::operator=(const HyperNet &src) {
    ...
    // Myanmar criteria-related
    min_dam_energy = src.min_firm_energy;
...
void HyperNet::readAdjList(std::ifstream& input, int num_edges) {
    ...
    for (int i = 0; i < num_edges; i++) {
        ...
        // Do the same for the seismic risk energy of the dam and other criteria
        ...
        // Do the same for the criteria specific to the Myanmar study
...
void HyperNet::readDamInfo(std::ifstream& input, int num_dams) {
    ...
    for (int i = 0; i < num_dams; i++){
        ...
        // Keeps track of minimum biodiversity
        ...
        // Keeps track of minimum ghg emissions
        if (newDam.ghg!= 0) {
            if (min_ghg == -1) {
                min_ghg = newDam.ghg;
            } else {
        ...
        // Keeps track of minimum deegree of regulation (DOR)
        if (newDam.dor!= 0) {
            if (min_dor == -1) {
                min_dor = newDam.dor;
            } else {
        ...
        // Keeps track of minimum population
        ...
        // Keeps track of minimum firm energy
        ...
        // Keeps track of minimum loss of agricultural land
        ...
        // Keeps track of minimum loss of forest land
...
void HyperNet::round_criteria(int node) {
    for (Edge& e: adj_lists[node]){
        ...
        // Round Myanmar-specific criteria
...
void HyperNet::generateBinaryTree() {
    ...
    if (balance_tree) { // Build a balanced tree
        generate_binary_tree_balanced(
            root_node, adj_lists[root], 0, (int) adj_lists[root].size() - 1, false, 
            nodes[root]);
    } else {
        // Allow for either splitting data values or not between intermediary nodes
        HyperNode root_data = nodes[root];
        if (split_values) {
            int num_intermediates_root = calculate_number_intermediate_nodes(
                adj_lists[root].size());
...
void
HyperNet::generate_binary_tree_balanced(
        HyperTreeNode *&node, std::vector<Edge>& children,
    ...
    if (!intermediate) {
        ...
        if (split_values) {
            split_node_data(
                node->node_data, 
                calculate_number_intermediate_nodes((int)children.size()));
        }
    ...
    if (high_ch_bound - low_ch_bound > 1) { // More than two children left so split and divide
        ...
        generate_binary_tree_balanced(
            node->left, children, low_ch_bound, median, true, node->node_data);
        ...
        if (high_ch_bound - (median + 1) == 0) { // Not intermediate
            ...
            generate_binary_tree_balanced(
                node->right, adj_lists[right_edge.endNode],
                0, (int)adj_lists[right_edge.endNode].size() - 1, false, 
                nodes[right_edge.endNode]);
        } else { // Need intermediate
            node->rightD.status = 2;
            node->rightD.decision = {0}; // The dam connecting a parent to an intermediate is never built
            generate_binary_tree_balanced(
                node->right, children, median + 1, high_ch_bound, true, 
                node->node_data);
        }
    } else {
        // Add real left child
        ...
        generate_binary_tree_balanced(
            node->left, adj_lists[left.endNode], 0, 
            (int)adj_lists[left.endNode].size() - 1,
        ...
        if (high_ch_bound - low_ch_bound > 0) {
            ...
            node->rightD = dams[right_edge.damId]; // Assign the right dam
            generate_binary_tree_balanced(
                node->right, adj_lists[right_edge.endNode], 0,
                (int) adj_lists[right_edge.endNode].size() - 1, false, 
                nodes[right_edge.endNode]);
        }
    }
