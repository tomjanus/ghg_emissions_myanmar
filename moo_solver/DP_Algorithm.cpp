...
// Modified to suit the Myanmar project by T. Janus on 17/01/2024
//

#include "DP_Algorithm.h"

DP_Algorithm::DP_Algorithm(const HyperNet &net, int root, 
                           const std::vector<std::string> &relevant_criteria, double epsilon,
                           unsigned int seed, bool use_n_squared, int batch_size, 
                           bool use_binary_tree, size_t num_threads)
        : net(net), root(root), relevant_criteria(relevant_criteria), k_epsilon(epsilon),
          random_seed(seed), use_n_squared(use_n_squared), batch_size(batch_size), 
...
void DP_Algorithm::computeDP_n_squared(int node) {
    ...
    if (childrenEdges.empty()) {
        ...
        this->pareto_partial_table[node].add_n_squared(
            criteria, node, {}, {}, (int) relevant_criteria.size(), false);
    } else {
        ...
        for (auto &decisionVec: decisionVectors) {
            for (auto &grouping: cartesian_pairs) {
                ...
                this->pareto_partial_table[node].add_n_squared(
                    criteria, node, decisionVec, grouping,
...
void DP_Algorithm::initialize_criteria(int node, std::pair<double, double> *criteria) {
    for (int i = 0; i < relevant_criteria.size(); i++) {
        if (relevant_criteria[i] == "connectivity") {
            ...
            relevant_criteria[i] == "firm_energy" || // Myanmar-specitic
            relevant_criteria[i] == "loss_agri" || // Myanmar-specitic
            relevant_criteria[i] == "loss_forest" // Myanmar-specitic
        ) {
            criteria[i] = {0, 0};
        } else {
...
void DP_Algorithm::update_criteria(int node, Pareto_Solution *pareto_decision, std::pair<double, double> *criteria,
    ...
    if (decision == 0) { // We are not building the dam
        ...
        // Variables for Myanmar-specific criteria
        ...
        for (int i = 0; i < relevant_criteria.size(); i++) {
            ...
            } else if (relevant_criteria[i] == "firm_energy") {     // MYANMAR-SPECIFIC
                // Add firm energy gained from built dam and firm enegry from child
                criteria[i].first += added_firm_energy + pareto_decision->criteria_2[i].first;
                // Add firm energy gained from built dam and rounded energy from child
            ...
            } else if (relevant_criteria[i] == "loss_agri") { // MYANMAR-SPECIFIC
                // Add the risk associated with the dam in its true value and that accumulated by child
            ...
            } else if (relevant_criteria[i] == "loss_forest") { // MYANMAR-SPECIFIC
                // Add the risk associated with the dam in its true value and that accumulated by child
...
void DP_Algorithm::computeDP_n_squared_generic(HyperTreeNode *node, bool round_values) {
    ...
    if (children_pairs.empty()) {
        ...
        node->local_frontier.add_n_squared(
            criteria, node->node_data.id, {}, {}, (int) relevant_criteria.size(), false);
    } else {
            ...
            for (auto &grouping: cartesian_pairs) {
                ...
                for (int i = 0; i < decisionVec.size(); i++) {
                    ...
                    update_criteria_new(
                        ...
                        population_built, firm_energy_built,
                        loss_agri_built, loss_forest_built);
                    //}
                }
                // Re-calculate the k used for energy and seismic for the given node
                if (!calc_built) {
                    assign_node_k(
                        node, energy_built, seismic_built, biodiversity_built, 
                        ghg_built, dor_built, population_built, firm_energy_built,
                        loss_agri_built, loss_forest_built);
                    calc_built = true;
                }
                ...
                node->local_frontier.add_n_squared(
                    criteria, node->node_data.id, decisionVec, grouping,
                    (int) relevant_criteria.size(), false);
            }
        }
    }
...
void DP_Algorithm::computeDP_nlogn_batching(HyperTreeNode *node) {
    ...
    if (node->left == nullptr && node->right == nullptr) {
        ...
        node->vec_frontier.push_back(
            new Pareto_Solution(
                node->node_data.id, criteria, {}, {}, (int) relevant_criteria.size()));
    } else { // Not leaf node
        ...
        if (node->left != nullptr && node->right != nullptr) { // Has both children
            ...
            if (node->right->vec_frontier.size()>2*K) {
                            ...
                            for (int left_dam_plan: node->leftD.decision) {
                                ...
                                for (Pareto_Solution *currR: partial_right) {
                                    for (Pareto_Solution *currL: node->left->vec_frontier) {
                                        ...
                                        add_nlogn_solution(
                                            pair, decision, node, solution_sets[i],
                                            num_generated, t,
                                            this->batch_number);
                                    }
                                }
                ...
                for(int i = 0; i < K; ++i) {
                    all_solution_set.insert(
                        solution_sets[i].begin(),solution_sets[i].end());
                    ...
                    for (int left_dam_plan: node->leftD.decision) {
                        ...
                        for (Pareto_Solution *currR: node->right->vec_frontier) {
                            for (Pareto_Solution *currL: node->left->vec_frontier) {
                                ...
                                // Call method to generate new solution
                                add_nlogn_solution(
                                    pair, decision, node, all_solution_set, 
                                    num_generated, t, batch_number);
                            }
                        }
            ...
            for (int left_dam_plan: node->leftD.decision) {
                ...
                for (Pareto_Solution* currL: node->left->vec_frontier) {
                    ...
                    // Call method to generate new solution
                    add_nlogn_solution(
                        pair, decision, node, all_solution_set, num_generated, t,
                        batch_number);
                }
            }
        ...
        // No ties exist
        std::vector<Pareto_Solution* > solution_set(
            all_solution_set.begin(), all_solution_set.end());
        t = clock() - t;
...
void DP_Algorithm::add_nlogn_solution(
        std::vector<Pareto_Solution *> &grouping, std::vector<int> &decisionVec,
        HyperTreeNode *node,
    ...
    auto adding = new Pareto_Solution(
        node->node_data.id, criteria, decisionVec, grouping, (int) relevant_criteria.size());
...
void DP_Algorithm::initialize_criteria(
            HyperTreeNode *node, std::pair<double, double> *criteria) {
    for (int i = 0; i < relevant_criteria.size(); i++) {
        if (relevant_criteria[i] == "connectivity") {
...
void DP_Algorithm::update_criteria_new(
        HyperTreeNode *node, Pareto_Solution *pareto_decision, std::pair<double, 
        double> *criteria, int decision, int child_index, double &energy_built, 
        double &seismic_built, double &biodiversity_built, double &ghg_built, 
        double &dor_built, double &population_built, double &firm_energy_built, 
    ...
    if (decision == 0) { // We are not building the dam
        for (int i = 0; i < relevant_criteria.size(); i++) {
            ...
            if(relevant_criteria[i] == "connectivity" ||
               ...
               relevant_criteria[i] == "population" ||
               relevant_criteria[i] == "firm_energy" ||
               relevant_criteria[i] == "loss_agri_built" ||
               relevant_criteria[i] == "loss_forest_built") {
                    // Add the true connectivity / sediment / energy of child
                    criteria[i].first += pareto_decision->criteria_2[i].first;
        ...
        for (int i = 0; i < relevant_criteria.size(); i++) {
            if (relevant_criteria[i] == "sediment") {
                ...
                // Add energy gained from built dam and rounded energy from child
                criteria[i].second += added_energy + pareto_decision->criteria_2[i].second;
            } else if (relevant_criteria[i] == "firm_energy") {     // Myanmar-study-specific
                // Add firm energy gained from built dam and energy from child
                criteria[i].first += added_firm_energy + pareto_decision->criteria_2[i].first;
                // Add firm energy gained from built dam and rounded energy from child
                criteria[i].second += added_firm_energy + pareto_decision->criteria_2[i].second;
            } else if (relevant_criteria[i] == "seismic") { // We should check this
...
void DP_Algorithm::assign_node_k(
        ...
        double biodiversity_built, double ghg_built, double dor_built, 
        double population_built, double firm_energy_built, double loss_agri_built,
        double loss_forest_built) {

    node->node_data.k_energy = std::floor(
        energy_built / net.min_dam_energy) * net.min_dam_energy * k_epsilon / 2.0;
    node->node_data.k_seismic = std::floor(
        seismic_built / net.min_seismic) * net.min_seismic * k_epsilon / 2.0;
    node->node_data.k_biodiversity = std::floor(
        biodiversity_built / net.min_biodiversity) * net.min_biodiversity * k_epsilon / 2.0;
    node->node_data.k_ghg = std::floor(
        ghg_built / net.min_ghg) * net.min_ghg * k_epsilon / 2.0;
    node->node_data.k_dor = std::floor(
        dor_built / net.min_dor) * net.min_dor * k_epsilon / 2.0;
    node->node_data.k_population = std::floor(
        population_built / net.min_population) * net.min_population * k_epsilon / 2.0;
    // Myanmar - specific
    node->node_data.k_firm_energy = std::floor(
        firm_energy_built / net.min_firm_energy) * net.min_firm_energy * k_epsilon / 2.0;
    node->node_data.k_loss_agri = std::floor(
        loss_agri_built / net.min_loss_agri) * net.min_loss_agri * k_epsilon / 2.0;
    node->node_data.k_loss_forest = std::floor(
        loss_forest_built / net.min_loss_forest) * net.min_loss_forest * k_epsilon / 2.0;
}
...
DP_Algorithm::divide_and_conquer(std::vector<Pareto_Solution *> &curr_set, int dimension) {
    ...
    std::sort(
        curr_set.begin(), 
        curr_set.end(), 
        Compare_dimensions(dimension, (int)this->relevant_criteria.size()));
    return divide_and_conquer_helper(curr_set, 0, curr_set.size() - 1, dimension);
}

std::vector<Pareto_Solution *>
DP_Algorithm::divide_and_conquer_helper(
        std::vector<Pareto_Solution *> &curr_set, unsigned long low, 
    ...
    std::sort(
        sup_inf.begin(), 
        sup_inf.end(), 
        Compare_dimensions(dimension - 1, (int)this->relevant_criteria.size()));
...
DP_Algorithm::marry(
    ...
    if (inf1 + inf2 == high - low + 1) { // All inferior so we return all!
        ...
        std::vector<Pareto_Solution* > immediate_drop(
            curr_set.begin() + low, curr_set.begin() + high + 1);
        // Sort and drop dimension on curr set
        std::sort(
            immediate_drop.begin(), immediate_drop.end(), 
            Compare_dimensions(dimension - 1, (int)this->relevant_criteria.size()));
    ...
    // Sort by d - 1 before calling marry
    std::sort(
        sup_first.begin(), sup_first.end(),
        Compare_dimensions(dimension - 1, (int)this->relevant_criteria.size()));

    std::vector<Pareto_Solution* > keep_inf_second = marry(
        sup_first, 0, sup_first.size() - 1, dimension - 1);
