...
#ifndef MYANMAR_PROJECT_PARETO_SOLUTION_H
#define MYANMAR_PROJECT_PARETO_SOLUTION_H
...
#define  MAX_CRITERIA 11 // Three additional criteria defined in the Myanmar case study (i.e. 8 + 3)
...
public:
    ...
    std::vector<std::pair<double, double> > criteria; 
    // NOTE: to allow generic types we should use std::variant
    ...
    bool inferior; // Used in the divide-and-conquer algorithm. Signals if the solution is in the 
                   // inferior or superior category
    ...
    Pareto_Solution* next; // Pointer to the next node in the linked list --- 
                           // Only used with linked list data structure.
     ...
     */
    Pareto_Solution(int node_id, const std::pair<double, double> criteria[],
                    const std::vector<int> &dam_decisions,
...
#endif //MYANMAR_PROJECT_PARETO_SOLUTION_H
