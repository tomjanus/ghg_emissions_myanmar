...
#ifndef MYANMAR_PROJECT_HYPERNET_H
#define MYANMAR_PROJECT_HYPERNET_H
...
struct HyperNode {
    ...
    double k_population;
    // Myanmar-specific
    double k_firm_energy;
    double k_loss_forest;
    double k_loss_agri;
    ...
    // Myanmar-specific
    double firm_energy_associated;
    double loss_forest_associated;
    double loss_agri_associated;
};
...
public:
     ...
     */
    HyperNet(const std::string &basin, const std::string &file,  bool split_values, 
    ...
    */
    ...
    std::vector<std::string> relevant_criteria; // Stores the criteria that we will be used for the analysis
    ...
    // Myanmar-specific
    double min_firm_energy;
    double min_loss_forest;
    double min_loss_agri;
    ...
    // Myanmar-specific
    double firm_energy_k;
    double loss_forest_k;
    double loss_agri_k;
...
private:
     ...
     *      Because we are adding intermediary nodes we must build 'dummy' dams connecting to 
     *      the children.
     */
    void generate_binary_tree_balanced(
        HyperTreeNode*& node, std::vector<Edge>& children, int low_ch_bound, 
        int high_ch_bound, bool intermediate, HyperNode& node_data);
     ...
     */
    void read_criterion(
        const std::string &criterion, std::stringstream &reader, HyperNode &newNode);
     ...
     * criterion represents the current criterion that is being read in.
     */
    void read_dam_criterion(
        const std::string& criterion, std::stringstream& reader, Dam& dam);
...
#endif //MYANMAR_PROJECT_HYPERNET_H
