...
public:
    DP_Algorithm(
        const HyperNet &net, int root, const std::vector<std::string> &relevant_criteria, 
        double epsilon, unsigned int seed, bool use_n_squared, int batch_size, 
        bool use_binary_tree, size_t num_threads); // Constructor
    // Constructor
    ...
    void update_criteria(
        int node, Pareto_Solution *pareto_decision, 
     ...
     */
    void add_nlogn_solution(
        std::vector<Pareto_Solution *> &grouping, std::vector<int> &decisionVec,
        HyperTreeNode *node, std::unordered_set<Pareto_Solution *, 
        SolutionHash, EqualSolutions> &solution_set,
     ...
     */
    void update_criteria(
        HyperTreeNode* node, Pareto_Solution *pareto_decision, 
        std::pair<double, double> *criteria,
        int decision, int child_index);
...
private:
     ...
     */
    template <typename T>
    std::vector<std::vector<T> > cartesian_product_generic(
    ...
    std::vector<Pareto_Solution* > divide_and_conquer(
        std::vector<Pareto_Solution* >& curr_set, int dimension);

    std::vector<Pareto_Solution* > divide_and_conquer_helper(
     ...
     */
    std::vector<Pareto_Solution* > marry(
        std::vector<Pareto_Solution *> &curr_set, unsigned long low,
                                       unsigned long high, int dimension);
