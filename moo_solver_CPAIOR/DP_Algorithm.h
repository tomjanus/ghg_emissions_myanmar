...
#ifndef MYANMAR_PROJECT_DP_ALGORITHM_H
#define MYANMAR_PROJECT_DP_ALGORITHM_H
...
// New function sued to compare solutions based on a given dimension for linear preference
// The input object should be in the format (energy, dot product 1, dot product 2...)
// TODO: Currently only support only 1 dot product
struct Compare_dimensions_lp {
    explicit Compare_dimensions_lp(int dimension, int total_dimensions) {
        this->dimension = dimension;
        this->total_dimensions = total_dimensions;
    }
    bool operator () (Pareto_Solution* sol1, Pareto_Solution* sol2) {
        for (int i = dimension - 1; i >= 0; i--) {
            if (sol1->criteria_lp_2[i].second == sol2->criteria_lp_2[i].second) {
                continue;
            }
            return sol1->criteria_lp_2[i].second > sol2->criteria_lp_2[i].second;
        }
        int tie = dimension;
        while (tie <= total_dimensions - 1) {
            if (sol1->criteria_lp_2[tie].second == sol2->criteria_lp_2[tie].second) {
                ++tie;
            } else {
                return sol1->criteria_lp_2[tie].second > sol2->criteria_lp_2[tie].second;
            }
        }
        return false;
    }
...
public:
    DP_Algorithm(
        const HyperNet &net, int root, const std::vector<std::string> &relevant_criteria, double epsilon,
        unsigned int seed, bool use_n_squared, int batch_size, bool use_binary_tree, 
        size_t num_threads, bool use_linear_preference, const std::vector<std::vector<double> > &w, std::string savename); // Constructor
    // Constructor
     ...
     */
    void assign_node_k(
            HyperTreeNode *node, double energy_built, double seismic_built, double biodiversity_built,
    ...
    void round_criteria_lp(HyperTreeNode* node, std::pair<double, double> *criteria);
...
private:
    ...
    // record the linear preference for all the criteria except for the energy
    ...
    // use function designed for merge nodes w.r.t linear function
    std::vector<Pareto_Solution* > divide_and_conquer_lp(std::vector<Pareto_Solution* >& curr_set, int dimension);
    std::vector<Pareto_Solution* > L2D_lp(std::vector<Pareto_Solution* >& all_possible);
    //------------------------------------
    ...
    std::vector<Pareto_Solution* > divide_and_conquer_helper_lp(std::vector<Pareto_Solution *> &curr_set,
                                                             unsigned long low, unsigned long high, int dimension);
    ...
    std::vector<Pareto_Solution* > marry_lp(std::vector<Pareto_Solution *> &curr_set, unsigned long low,
                                         unsigned long high, int dimension);
    ...
    std::vector<Pareto_Solution* > marry_2d_lp(std::vector<Pareto_Solution* >& curr_set);
...
#endif //MYANMAR_PROJECT_DP_ALGORITHM_H
